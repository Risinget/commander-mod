...
public class ScreenshotRecorderMixin {
    ...
    public static void grab(File gameDirectory, @Nullable String fileName, RenderTarget framebuffer, int downscaleFactor, Consumer<Component> messageReceiver) {
            ...
            Util.ioPool().execute(() -> {
                try {
                    ...
                    try {
                        ...
                        Component text = FormatterUtils.parseAndFormatText(Prefix.COMMANDER+" ").append(Component.literal(file3.getName()).withStyle(ChatFormatting.UNDERLINE).withStyle((style) -> style.withClickEvent(new ClickEvent.CopyToClipboard(file3.getAbsolutePath()))));
                        messageReceiver.accept(Component.translatable("screenshot.success", new Object[]{text}));
                        if(ConfigCommander.getEnableUploadToCloud()){
                            try {
                                CloudinaryScreenshot.uploadImage(String.valueOf(file3));
                                messageReceiver.accept(Component.literal("Enviado a la nube: "+file3.getName()));
                            }catch (Exception e){
                                messageReceiver.accept(Component.literal("Error al subir la imagen: "+e.getMessage()));
                            }
                        }
                    ...
                    messageReceiver.accept(Component.translatable("screenshot.failure", new Object[]{exception.getMessage()}));
                }
